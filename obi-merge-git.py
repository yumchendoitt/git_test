...
try:
    ...
    arg_parser.add_argument('action', choices=['startFeature', 'finishFeature', 'refreshFeature', 'startRelease',
                                               'finishRelease', 'bugfix', 'startHotfix', 'finishHotfix', 'startFetch',
                                               'finishFetch', 'fetch', 'resumeMerge', 'checkSetup', 'finishFeatureBug'],
                            help='Gitflow action.')
    arg = '?' if sys.argv[1] in ('finishFetch', 'fetch', 'resumeMerge', 'checkSetup') else None
    arg_parser.add_argument('name', nargs=arg, help='Name of a feature, release or hotfix depending on the action chosen.')
    arg_parser.add_argument('-p', '--push', action="store_true", default=True, help='Push directly to origin.')
    ...
    arg_parser.add_argument('-s', '--schedule', default=False,
                            help='Git fetch in a time interval')
    ...
    TASK_NAME = 'FETCH'
    SCHEDULE = NAME if ACTION == 'startFetch' else args.schedule

    if (ACTION == 'startFeature' or ACTION == 'finishFeature' or ACTION == 'refreshFeature' or ACTION == 'bugfix' or
            ACTION == 'startFetch') and NAME is None:
...
def branch(branch_name, base):
    ...
    kill_process("admintool.exe")
    checkout(base)
    pull()
    out = cmd(['checkout', '-b', branch_name, base])
    push(GIT_REMOTE, branch_name, True)
    return out
...
def merge(trunk, branch_name, no_ff=True):
...
def push(remote, branch_name, up_stream=False):
    """Pushes a branch to a remote repository."""
    print('Pushing %s to %s...' % (branch_name, remote))
    if up_stream:
        out = cmd(['push', '--set-upstream', remote, branch_name])
        return out
    else:
...
def kill_process(process_name):
    ...
    try:
        for task_line in check_output("tasklist").splitlines():
            task = task_line.split()
            if len(task) > 0 and task[0] == process_name:
                os.system("taskkill /f /im  %(process_name)s" % locals())
                break
    except Exception, e:
        print(e)
...
def create_patch(orig_rpd, orig_pass, curr_rpd, curr_pass, patch_file):
    ...
    script = [bi_command('comparerpd'), '-C', curr_rpd, '-p', curr_pass, '-G', orig_rpd, '-W', orig_pass, '-D',
              patch_file]
...
def start_fetch():
    command = 'SCHTASKS /Create /SC MINUTE /MO %s /TN %s /TR "python %s\\obi-merge-git.py fetch"' % (SCHEDULE, TASK_NAME, SCRIPT_DIR)
    print command
...
def finish_fetch():
    command = 'SCHTASKS /Delete /TN %s /F' % TASK_NAME
    output = Popen(command, stdout=PIPE, stderr=PIPE, shell=True).communicate()
    if output[0]:
        print output[0]
    elif output[1]:
        if str(output[1]).__contains__('Access is denied'):
            print 'ERROR: Please run as administrator'
        else:
            print 'No Task need to delete'
...
def finish_feature(feature):
    feature_name = FEATURE_PREFIX + feature
    push(GIT_REMOTE, feature_name)
    response = git_bi_merge(GIT_DEVELOP, feature_name)
    if response:
        merge_success(GIT_DEVELOP, feature_name)
    finish_fetch()
...
# use only after resume_merge()
def finish_feature_after_resume_merge(feature):
    feature_name = FEATURE_PREFIX + feature
    push(GIT_REMOTE, feature_name)
    merge(GIT_DEVELOP, feature_name)
    merge_success(GIT_DEVELOP, feature_name)


# This is used when merge process stop at a point where a.rpd b.rpd and c.rpd are created
def resume_merge():
    cleanup_rpd_files(GIT_REPO)
...
def check_setup():
    ...
    if not os.path.exists(BIINIT_PATH):
        if not os.path.exists(OBIEE_HOME):
            ...
            setup_result += '%s: admintool is not found in %s\n' % ("OBIEE_HOME", OBIEE_HOME)
    if RPD_PW == '':
        setup_result += 'RPD_PW: Please provide rpd password\n'
    print 'config.ini is GOOD TO GO' if len(setup_result) == 0 else setup_result
...
def main():
    ...
    elif ACTION == 'startFetch':
        start_fetch()
    elif ACTION == 'finishFetch':
        finish_fetch()
    elif ACTION == 'fetch':
        fetch()
    elif ACTION == 'resumeMerge':
        resume_merge()
    elif ACTION == 'checkSetup':
        check_setup()
    elif ACTION == 'finishFeatureBug':
        finish_feature_after_resume_merge(NAME)
